...
class Tracts {
  ...
  */
  constructor(network) {
    _defineProperty(this, "tracts", []);
    // Tract
    _defineProperty(this, "tractPaths", {});
    // tractPaths[ <path> ] = Tract
    _defineProperty(this, "connections", {});
    // actual connections with Float32Array per variable
    _defineProperty(this, "connectionsLen", 0);
      ...
      for (let i = 0; i < connectionLayout.length; i++) {
        ...
        for (const [key, value] of Object.entries(conn)) {
          if (key !== "from" && key !== "to") {
            ...
            if (!found) {
              throw new Error("Trying to make a connection with tract variable: " + key + ", which was not defined in the paradigm");
            }
          }
        }
        // check if connection has at least "from" and "to"
        if (!("from" in conn)) {
          throw new Error("Connection has no 'from' value defined");
        }
        if (!("to" in conn)) {
          throw new Error("Connection has no 'to' value defined");
        }
